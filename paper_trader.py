...
class PaperTrader:
    ...
    def __init__(self, cash=10000.0, risk_pct=RISK_DEFAULT, stop_pct=STOP_DEFAULT,
                 fee_rate=0.001, log_path=None, trail_pct=None):
        self.cash = cash
        self.risk_pct = risk_pct
        self.stop_pct = stop_pct          # 仓位尺子: 名义仓位 = risk/stop
        self.fee_rate = fee_rate
        self.trail_pct = trail_pct or None  # 跟踪止损比例(只上移), None=关闭
        self.units = 0.0
        ...
        self.high_watermark = 0.0          # 持仓区间最高价, 跟踪止损用
        self.slippage_expected = 0.0       # 最近一次买入的期望价(卖出日志用); 必须有初值
        self.signal = 0.0
    ...
    def buy(self, price, reason, ts, notional=None):
        exec_price = price
        stop_dist = exec_price * self.stop_pct
        if notional is None:  # 默认按风险尺子(自动盘用); 手动下单可指定投入金额
            notional = self.cash * self.risk_pct / self.stop_pct
        notional = min(notional, self.cash)  # 不许花超过手上的现金
        ...
        self.high_watermark = exec_price
        # 跟踪止损: 初始线取"固定止损 与 跟踪止损 更紧的一档"
        if self.trail_pct:
            self.stop_price = max(self.stop_price, exec_price * (1 - self.trail_pct))
        self.entry_time = ts
    ...
    def sell(self, price, reason, ts, pnl_note=True):
        ...
        _units_sold = self.units
        self.units = 0.0
        self.stop_price = np.inf
        self.high_watermark = 0.0
        try:  # 先清仓再记日志: 日志失败也绝不留下"钱加了仓没清"的坏账
            self.log_row(ts, "SELL", reason, self.slippage_expected or exec_price,
                         exec_price, _units_sold, pnl_acct)
        except Exception:
            pass
        return pnl_acct

    def update_stop(self, price):
        """跟踪止损: 只上移。持仓且开启 trail_pct 时, 每次用最新价刷新止损线。"""
        if self.trail_pct and self.units > 0 and price > 0:
            if price > self.high_watermark:
                self.high_watermark = price
                new_stop = self.high_watermark * (1 - self.trail_pct)
    ...
    def log_row(self, ts, side, reason, expected, fill, units, pnl_acct=0.0):
        ...
        # 卖出时仓位已经清零、现金也已经到账, 再加一次仓位市值会把余额算高一截
        # (之前卖完一笔 13 USDT 的仓位, 日志余额会显示成 113, 看着像凭空赚了钱)
        bal = self.cash if side == "SELL" else self.cash + units * fill
        with self.log_path.open("a", encoding="utf-8", newline="") as f:
            w = csv.writer(f)
            w.writerow([
                ts, side, reason, f"{expected:.2f}", f"{fill:.2f}", f"{slip:+.4%}",
                f"{units * fill:.2f}", f"{units:.8f}", f"{pnl_acct:+.4%}", f"{bal:.2f}",
            ])
